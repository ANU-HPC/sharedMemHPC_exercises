...
int main(int argc, char *argv[]) {
  ...
  printf("\n\n");
  printf(
      "Transpose  Version             NSIZE    REPEAT    Time(sec)        "
      "MFLOPS         Error\n");
  printf(
      "------------------------------------------------------------------------"
      "---------------\n");
  ...
  for (trans = 0; trans < 2; trans++) {
    for (vers = 0; vers < NVERSIONS; vers++) {
      ...
      char *vers_describe[NVERSIONS] = {"Dot", "Daxpy", "Dot (unrolled)",
                                        "Daxpy (unrolled)"};
      printf("%-11s%-19s%6i%9i%14.6f%14.6f%14.2e\n",
             trans ? "Transpose" : "Normal", vers_describe[vers], nsize, repeat,
             tmatvec, mflops, err);
...
void matvecmult(int vers, int trans, int n, double a[], double b[],
  ...
  switch (vers) {
    case 0:
      ...
      if (trans != 0) {
        for (i = 0; i < n; i++) mydot(&x[i], n, &a[i], n, b, 1);
      } else {
        for (i = 0; i < n; i++) mydot(&x[i], n, &a[i * n], 1, b, 1);
      }
      break;
    case 1:
      // daxpy based matrix vector product
      if (trans != 0) {
        for (i = 0; i < n; i++) mydaxpy(n, b[i], x, 1, &a[i * n], 1);
      } else {
        for (i = 0; i < n; i++) mydaxpy(n, b[i], x, 1, &a[i], n);
      }
      break;
    case 2:
      // unrolled dot product based matrix vector product
      if (trans != 0) {
        for (i = 0; i < n / 2; i++)
          mydot_u2(&x[2 * i], n, &a[2 * i], n, b, 1, 1);
        if (n % 2 == 1) mydot(&x[n - 1], n, &a[n - 1], n, b, 1);
      } else {
        for (i = 0; i < n / 2; i++)
          mydot_u2(&x[2 * i], n, &a[2 * i * n], 1, b, 1, n);
        if (n % 2 == 1) mydot(&x[n - 1], n, &a[(n - 1) * n], 1, b, 1);
      }
      break;
    case 3:
      // unrolled daxpy based matrix vector product
      if (trans != 0) {
        for (i = 0; i < n / 2; i++)
          mydaxpy_u2(n, &b[2 * i], x, 1, &a[2 * i * n], 1, n);
        if (n % 2 == 1) mydaxpy(n, b[n - 1], x, 1, &a[(n - 1) * n], 1);
      } else {
        for (i = 0; i < n / 2; i++)
          mydaxpy_u2(n, &b[2 * i], x, 1, &a[2 * i], n, 1);
        if (n % 2 == 1) mydaxpy(n, b[n - 1], x, 1, &a[n - 1], n);
      }
      break;
    default:
